...
- PlayStation/DualSense 状态灯是白色 player LEDs，不支持颜色，也不支持线性亮度；亮度只按 3 个梯度处理。
...
所有开发任务必须先阅读本指南再修改代码。Agent 或开发者开始任务时，先做任务历史状态读取，确认当前分支、基线、相关功能点、已知限制和历史决策；随后在“当前开发状态”中记录正在做的任务。开发完成后必须回写完成状态、测试结果、未解决问题和下一步，确保本指南始终反映项目真实进度。

状态灯规则是硬性约束：PlayStation/DualSense 状态灯是白色 player LEDs，不支持颜色，不支持线性亮度，亮度只允许 3 档梯度。
...
## 功能 1：测试页改为左侧操作栏 + 右侧手柄预览

### 目标

把 `/test` 从纵向堆叠页面改成工作台式布局。左侧集中放硬件状态、输出控制、按键测试和发包日志；右侧显示大尺寸 DualSense 手柄预览图、触控板手指位置、摇杆/扳机状态、陀螺仪和加速度计。用户调试硬件时，不需要上下滚动寻找事件，也能直接看见手柄输入反馈。

### 当前状态

- `APIServer.testPageHTML()` 内嵌整个测试页 HTML/CSS/JS。
- 当前页面已改为左侧 `.side-panel` 和右侧 `.preview-panel`。
- 发包日志在左侧 `#packetLog`，按时间向下追加并自动滚动。
- 右侧 SVG 手柄预览使用 `[data-button]` 元素高亮按钮。
...
1. 硬件状态
2. 输出控制：麦克风灯、灯带、玩家状态灯、3 档状态灯亮度、重/轻马达、左右自适应扳机
3. 按键测试
4. 发包日志

右侧预览模块：

- 内嵌 SVG 绘制 DualSense 轮廓和按钮，不依赖外部图片。
- 每个按钮使用独立 `[data-button]` 标识，按下时添加 `.active`。
- 触控板区域显示 1-2 个半透明手指圆点和坐标。
- 摇杆小圆点按 `hid.axis` 偏移。
- L2/R2 区域按扳机模拟量显示强度。
- 手柄图下方显示陀螺仪 raw X/Y/Z 和加速度计 raw X/Y/Z。
...
- 中间日志列表：
  - 每条日志一行摘要。
  - 显示事件类型和 JSON payload。
  - 高频 motion/axis/touch 输入做节流，避免淹没发包日志。
  - 保留最近 500 条，自动滚动到底部。
...
1. 编译并启动服务。
2. 打开 `/test`。
3. 确认左侧是操作栏和发包日志，右侧是手柄预览图。
4. 点击灯光按钮，左侧日志立即增加 `ui.action` 和 `hid.output.*`。
5. 按手柄按钮，右侧 SVG 对应按钮高亮，日志可按“输入”过滤查看。
6. 切换过滤器，日志数量和内容正确变化。
...
稳定控制 DualSense 下方 5 颗白色 player LEDs 的组合亮灭，并支持硬件提供的 3 档亮度。状态灯不支持颜色，也不支持线性亮度，不做伪线性滑块或呼吸动画。

### 当前状态

- 当前基线普通控制只写 `mask`。
- UI 已禁用状态灯线性亮度滑块；后续只允许做 3 档亮度枚举控件。
...
后端普通控制可以忽略 brightness；如果开放亮度控制，只接受 3 档枚举值。
...
- 不支持状态灯呼吸动画。
- 不支持状态灯颜色字段。
- 如果前端提供亮度选择，应使用 select 或 segmented control，不使用 range 线性滑块。
...
- 状态灯 player LEDs 是白色指示灯。
- 状态灯不支持颜色。
- 状态灯不支持线性亮度，只支持 3 个亮度梯度。
...
| 2026-06-17 | 状态灯普通控制只写 mask | brightness 字段会干扰实际效果 | 亮度控制只允许作为 3 档枚举补充 |
| 2026-06-17 | 状态灯不支持颜色和线性亮度 | PlayStation/DualSense player LEDs 是白色状态指示灯 | UI 不提供颜色选择器、线性滑块或呼吸动画 |
