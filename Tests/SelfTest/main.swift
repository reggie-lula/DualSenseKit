...
struct SelfTest {
    static func main() throws {
        ...
        mapper.secondaryBegan(x: 0.2, y: 0.2)
        mapper.primaryMoved(x: 0.1, y: 0.1, config: touchpad)
        mapper.primaryMoved(x: 0.2, y: 0.2, config: touchpad)
        expect(poster.moves.count == 2, "primary movement should be ignored while secondary touch is active")
        mapper.secondaryMoved(x: 0.2, y: 0.2, config: touchpad)
        mapper.secondaryMoved(x: 0.2, y: 0.3, config: touchpad)
        expect(poster.scrolls.count == 1, "secondary movement should scroll instead of moving cursor")
        mapper.resetSecondary()
        mapper.primaryMoved(x: 0.4, y: 0.4, config: touchpad)
        expect(poster.moves.count == 2, "primary should reseed after secondary touch ends")
        mapper.primaryMoved(x: 0.5, y: 0.4, config: touchpad)
        expect(poster.moves.count == 3, "primary should move again after secondary touch ends and reseeds")
